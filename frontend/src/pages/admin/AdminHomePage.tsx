...
import { Building2, Clock3, LoaderCircle, MapPin, RefreshCw, UserRound, Users } from "../../ui/icons";
...
export function AdminHomePage() {
  ...
  const onboardingSteps = [
    ...
    },
    {
      ...
      action: (e: MouseEvent<HTMLAnchorElement>) => go(e, "/app/admin/utilisateurs?status=pending"),
    },
    {
      id: 4,
  ...
  return (
    <div className="admin-home">
      ...
      <div className="home-header">
        <div>
          <p className="home-welcome">Bonjour, {session.user.fullName.split(" ")[0]} 👋</p>
          <p className="home-subtitle">Aperçu général et configuration de votre réseau uSwap</p>
        </div>
        <div className="home-actions">
          <a className="admin-button secondary small" {...link("/app/admin/stations?tab=list")}>
            Gérer les stations
          </a>
          <button type="button" className="admin-button primary-cta small" onClick={() => setShowCreateModal(true)}>
      ...
      <section className="network-overview" aria-label="Gestion du réseau">
        <a className="network-card card-accent-navy" {...link("/app/admin/utilisateurs")} aria-label="Gérer les utilisateurs">
        ...
        <a className="network-card card-accent-blue" {...link("/app/admin/stations?tab=list")} aria-label="Consulter les stations">
          <div className="network-card-head">
        ...
        <a className={`network-card ${counts.pending > 0 ? "card-accent-amber warning" : "card-accent-neutral"}`} {...link("/app/admin/utilisateurs?status=pending")}>
          <div className="network-card-head">
      ...
      {!dismissOnboarding && (
        <section className="onboarding-card" aria-label="Progression de la configuration">
          <button type="button" className="onboarding-dismiss-btn" aria-label="Masquer l'onboarding" onClick={() => setDismissOnboarding(true)}>
            ✕
          </button>
          <div className="onboarding-head">
            ...
            <div className="onboarding-progress-wrap">
              ...
              <div className="onboarding-progress-bar">
                <div className="onboarding-progress-fill" style={{ width: `${(completedCount / onboardingSteps.length) * 100}%` }} />
              </div>
            </div>
          </div>
          <div className="onboarding-grid">
            {onboardingSteps.map((step) => {
              ...
              return (
                <div key={step.id} className={`onboarding-step-item ${step.completed ? "is-completed" : "is-active"}`}>
                  <div className="step-header-line">
                    <span className="step-status-indicator">
                      {step.completed ? <span className="check-badge">✓</span> : <span className="radio-badge">{step.id}</span>}
                    </span>
                  ...
                  {!step.completed &&
                    (step.href ? (
                      <a className="admin-button secondary small step-btn" href={step.href} onClick={step.action as (e: MouseEvent<HTMLAnchorElement>) => void}>
                        {step.ctaLabel}
                      </a>
                    ) : (
                      <button type="button" className="admin-button secondary small step-btn" onClick={step.action as () => void}>
                        {step.ctaLabel}
                      </button>
                    ))}
                </div>
      ...
      {counts.pending > 0 && (
        <section className="pending-alert-banner">
          ...
          <a className="admin-button secondary small" {...link("/app/admin/utilisateurs?status=pending")}>
            Activer les accès
          </a>
        </section>
      )}
