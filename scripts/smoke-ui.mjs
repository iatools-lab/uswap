...
async function createPlanning(page, { mode, name, start, end }) {
  ...
  await page.getByRole("heading", { name: "Station Bastos", exact: true }).waitFor();
  await page.getByText("Repos minimal", { exact: true }).waitFor();
  await page.getByText(/swappeurs?$/).first().waitFor();
  const stationSelectorBox = await page.locator(".stepper-field-group").first().boundingBox();
  const stationSummaryBox = await page.locator(".planner-station-summary").boundingBox();
  assert.ok(stationSelectorBox && stationSummaryBox, "Le résumé de station doit être visible");
  assert.ok(
    stationSummaryBox.y - (stationSelectorBox.y + stationSelectorBox.height) < 40,
...
await exercise(
  ...
  async (page) => {
    ...
    const notificationBox = await page.locator(".notification-bell__trigger").boundingBox();
    const accountBox = await page.locator(".account-trigger").boundingBox();
    assert.ok(notificationBox && accountBox, "Les actions du compte doivent être visibles");
    assert.ok(accountBox.x > notificationBox.x, "La cloche doit précéder l'avatar");
    assert.ok(accountBox.x - (notificationBox.x + notificationBox.width) < 20, "La cloche doit rester accolée à l'avatar");
...
await exercise(
  ...
  async (page) => {
    ...
    const replaceButtons = page.getByRole("button", { name: "Affecter", exact: true });
    if (await replaceButtons.count()) {
      await replaceButtons.first().click();
      await page.getByRole("heading", { name: "Affecter un remplaçant" }).waitFor();
      await page.getByRole("button", { name: "Fermer" }).click();
    }
    await page.getByRole("tab", { name: "Historique" }).click();
    await page.getByRole("heading", { name: "Historique des changements d’affectation" }).waitFor();
    await page.getByRole("link", { name: "Plannings", exact: true }).click();
...
await exercise(
  ...
  async (page) => {
    ...
    await page.getByRole("heading", { name: "Générer un QR de service" }).waitFor();
    await page.getByRole("button", { name: "Fin de service", exact: true }).click();
    await page.getByRole("button", { name: "Shift publié pour le QR" }).click();
    const qrOptions = page.getByRole("option");
    assert.ok(await qrOptions.count(), "Un shift courant de la station doit permettre de générer un QR");
    await qrOptions.first().click();
    await page.getByRole("button", { name: "Générer le QR de fin" }).click();
    await page.getByRole("img", { name: "QR temporaire de pointage" }).waitFor();
    await page.getByRole("dialog").getByRole("button", { name: /Fermer/ }).click();
    await page.getByRole("heading", { name: /Présence du jour/ }).waitFor();
    await page.getByRole("heading", { name: /Pointages/ }).last().waitFor();
    await page.getByRole("link", { name: "Plannings", exact: true }).click();
    await page.waitForURL(/\/app\/station\/plannings$/);
    await page.getByText(/Semaine opérationnelle|Du .* au/).first().waitFor();
...
await exercise(
  ...
  async (page) => {
    ...
    for (const name of ["Générer un planning", "Nouveau planning"]) {
      ...
      assert.ok(box.x >= -1 && box.x + box.width <= 391, `Action hors écran : ${name}`);
    }
