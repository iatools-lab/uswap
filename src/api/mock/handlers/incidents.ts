...
function serialize(
  ...
  return {
    ...incident,
    stationName: stationNameOf(db, incident.stationId) ?? "Station inconnue",
...
export const incidentRoutes: MockRoute[] = [
  {
    ...
    handler: ({ db, user, query }) => {
      ...
      const meanHours = resolved.length
        ? Math.round(
            (resolved.reduce(
              (sum, item) =>
                sum +
                (Date.parse(item.resolvedAt!) - Date.parse(item.createdAt)) /
                  3600000,
              0,
            ) /
              resolved.length) *
      ...
      return {
        incidents: rows.map((item) => serialize(db, item)),
        metrics: {
          total: rows.length,
          open: rows.filter(
  ...
  },
  {
    ...
    handler: ({ db, user, body, now }) => {
      ...
      const incident: MockIncident = {
        ...
        description,
        attachmentIds: [],
        occurredAt: String(body.occurredAt || isoFromMs(now)),
  ...
  },
  {
    ...
    handler: ({ db, user, body, params, now }) => {
      ...
      const incident = db.incidents.find((item) => item.id === params[0]);
      if (!incident) throw new MockHttpError(404, "Incident introuvable.");
      const nextStatus = String(
        body.status ?? incident.status,
      ) as IncidentStatus;
      if (
        nextStatus !== incident.status &&
        !allowedTransitions[incident.status].includes(nextStatus)
      )
        throw new MockHttpError(
          409,
      ...
      if (["RESOLVED", "CLOSED"].includes(nextStatus) && comment.length < 10)
        throw new MockHttpError(
          400,
      ...
      incident.updatedAt = isoFromMs(now);
      if (body.severity) incident.severity = body.severity as IncidentSeverity;
      if (body.assigneeId !== undefined)
        incident.assigneeId = String(body.assigneeId) || null;
      if (nextStatus === "RESOLVED") {
        incident.resolvedAt = isoFromMs(now);
