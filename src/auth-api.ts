...
export async function api<T>(path: string, body?: unknown, method?: 'PATCH'): Promise<T> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new ApiError(response.status, typeof detail?.message === 'string' ? detail.message : response.status === 413 ? 'Le fichier dépasse la limite de 2 Mo.' : messages[response.status] || 'Le service est momentanément indisponible.');
    }
...
export async function download(path: string, filename: string): Promise<void> {
  ...
  try {
    ...
    if (!response.ok) {
      const messages: Record<number, string> = { 401: 'Identifiants invalides ou session expirée.', 403: 'Vous ne disposez pas des droits nécessaires.', 429: 'Trop de tentatives. Veuillez patienter avant de réessayer.', 503: 'Ce service est momentanément indisponible. Veuillez réessayer plus tard.' };
