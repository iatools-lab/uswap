...
function NotificationPreferences(){
 ...
 async function channel(channel:'emailEnabled'|'pushEnabled',value:boolean){
   if(channel==='pushEnabled'&&value&&'Notification' in window){
     const permission=await Notification.requestPermission();
     if(permission!=="granted") value=false;
     else new Notification("Notifications uSwap activées",{body:"Vous recevrez ici les alertes opérationnelles autorisées.",icon:"/icons/app-192.png"});
   }
   setPreferences(current=>current?{...current,[channel]:value}:current);setSaved(false)
 }
